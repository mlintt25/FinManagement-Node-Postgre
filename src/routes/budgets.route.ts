...
import {
  ...
  updateUserBudgetValidator
} from '~/middlewares/budgets.middleware'
 ...
 * @method PATCH
 * @header { Authorization: Bearer <access_token> }
 * @body { id: string, money_accounts: string[], transaction_type_categories: string[], name: string,
 *  amount_of_money: number, start_date: date, end_date: date }
 * @returns {Object} Response object with message.
 */
budgetsRouter.patch(
  ...
  accessTokenValidator,
  updateUserBudgetValidator,
  wrapRequestHandler(updateUserBudgetController)
)
